...
public class Environment {
    ...
    public List<Vector2Int> GetPath(Vector2Int from, Vector2Int to) {
        var open = new HashSet<Vector2Int>{from};

        System.Func<Vector2Int, int> h = (Vector2Int tile) => Mathf.Abs((to - tile).x) + Mathf.Abs((to - tile).y);
        var f = new Dictionary<Vector2Int, int>{{from, h(from)}};
        var g = new Dictionary<Vector2Int, int>{{from, 0}};
        ...
        while (open.Count > 0) {
            ...
            if (current == to) {
                ...
                while (parent.ContainsKey(current) && parent[current] != from) {
            ...
            foreach (var neighbor in new[]{current + Vector2Int.left,current + Vector2Int.up,current + Vector2Int.right,current + Vector2Int.down}) {
                if (!IsWalkable(neighbor) || GetUnit(neighbor) != null)
                    continue;
